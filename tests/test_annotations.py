...
def test_short_comment_types(create_project, run):
    ...
    '''
        ...
        assert type_reprs == [repr(str), repr(int)]
...
@pytest.mark.parametrize(
    "type_, expected", [("AnyStr", str), ("str", str), ("ByteString", bytes)]
)
...
@pytest.mark.parametrize("type_", ["int", "float", "complex"])
...
def test_sequence_types(create_project, run, type_, expected_type, expected):
    ...
    '''.format(
        ...
        assert reprs == [expected, repr(expected_type)]
...
def test_function_as_type(create_project, run):
    ...
    '''
        ...
        type_reprs = (
            run("say types '{\"1\": 1}'", stderr=True).strip().split("\n")
        )
        type_reprs[0] = eval(type_reprs[0])
        assert type_reprs == [{"1": 1}, repr(dict)]
